...
def make_device(name: str, role: DeviceRole) -> DeviceRecord:
    ...
    We keep identity and endpoints as simple placeholders.
    Topology validation only cares about roles and links.
    """
    return DeviceRecord(
        name=name,
        role=role,
...
def test_two_tier_topology_passes_basic_validation():
    ...
    Topology
    leaf1 connects to spine1 and spine2 using fabric links
    leaf2 connects to spine1 and spine2 using fabric links

    Expected
    validate_clos_topology returns ok True and no errors
    """
...
def test_leaf_with_one_uplink_fails_validation():
    """
    Build an invalid topology where a leaf has only one fabric uplink to spines.

    Expected
    validate_clos_topology returns ok False and an error mentioning at least 2 uplinks
    """
...
def test_three_tier_super_spine_rules():
    ...
    Expected
    validate_clos_topology returns ok True
    """
